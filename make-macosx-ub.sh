...
BIN_OBJ="
	build/release-darwin-ppc/openarena-smp.ppc
	build/release-darwin-i386/openarena-smp.i386
"
BIN_DEDOBJ="
	build/release-darwin-ppc/oa_ded.ppc
	build/release-darwin-i386/oa_ded.i386
"
BASE_OBJ="
...
(ARCH=ppc CFLAGS=$PPC_CFLAGS LDFLAGS="$PPC_LDFLAGS -L./libs" make -j$NCPU) || exit 1;

# intel client and server
if [ -d build/release-darwin-i386 ]; then
	rm -r build/release-darwin-i386
fi
(ARCH=i386 CFLAGS=$X86_CFLAGS LDFLAGS="$X86_LDFLAGS -L./libs" make -j$NCPU) || exit 1;
...
echo "
	...
	<dict>
		...
		<string>OpenArena $Q3_VERSION</string>
