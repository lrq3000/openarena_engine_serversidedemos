...
void CL_AdjustAngles( void ) {
	...
	if ( !in_strafe.active ) {
		cl.viewangles[YAW] -= speed*140.0*CL_KeyState (&in_right);
		cl.viewangles[YAW] += speed*140.0*CL_KeyState (&in_left);
	}

	cl.viewangles[PITCH] -= speed*140.0 * CL_KeyState (&in_lookup);
	cl.viewangles[PITCH] += speed*140.0 * CL_KeyState (&in_lookdown);
}
