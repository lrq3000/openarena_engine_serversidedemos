...
void SV_DemoStartPlayback(void)
{
	...
	clients = MSG_ReadBits(&msg, CLIENTNUM_BITS); // this is what produces the map bug with playing back the new demos, because of the offset
	// TOFIX: autoset the democlients and maxclients vars, but it needs some adjustments, for now it only set them to 0!
