...
int start, end;
	...
	ent = &g_entities[0];
	for( i = 0; i < ( level.demoState == DS_PLAYBACK ? g_maxclients.integer : level.num_entities ); i++, ent++ ) { // optimization for CPU and memory when replaying a demo, but also works well without
		if ( !ent->inuse ) {
			continue;
		}
