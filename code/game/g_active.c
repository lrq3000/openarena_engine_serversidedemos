...
void SpectatorThink( gentity_t *ent, usercmd_t *ucmd ) {
	...
	if ( client->sess.spectatorState != SPECTATOR_FOLLOW && !cl->pers.demoClient ) {
...
void SpectatorClientEndFrame( gentity_t *ent ) {
	...
	if ( ent->client->sess.spectatorState == SPECTATOR_FOLLOW ) {
		...
		if ( clientNum >= 0 ) {
			...
			if ( (cl->pers.connected == CON_CONNECTED || cl->pers.demoClient) && cl->sess.sessionTeam != TEAM_SPECTATOR ) {
